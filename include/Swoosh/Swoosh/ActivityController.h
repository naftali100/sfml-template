...
public:
    /**
      @brief constructs the activity controller, sets the virtual window size to the window, and initializes default
      values
    */
    ...
    /**
      @brief constructs the activity controller, sets the virtual window size to the user's desired size, and
       ...
       */
        ...
        public:
            ...
            */
            ...
            /**
              @brief This will start a REWIND state for the activity controller and creates a segue object onto the
              stack
              @return True if the target activity type T is found in the stack, false otherwise. Found activities
              transform into segues.
    ...
    */
    ...
    /**
    @brief Immediately replaces the current activity on the stack with a segue or activity depending on the resolved
    class type
    */
    template <typename T, typename... Args>
    ...
    /**
      @brief Tries to pop the activity of the stack that may be replaced with a segue to transition to the previous
      activity on the stack
      @return true if we are able to pop, false if there less than 1 item on the stack or in the middle of a segue
      effect
    */
    template <typename T>
    const bool pop() {
    ...
    /**
     ...
     @return true if we are able to pop, false if there are no more items on the stack or in the middle of a segue
     effect
   */
    const bool pop() {
    ...
    */
    ...
    @brief If type is not a segue, looks for the matching activity. If it is not found, it does not rewind to that
    activity.
...
private:
   ...
   */
    ...
    /**
       @brief When pop() is invoked, the pop is not executed immediately. It is deffered until it is safe to pop the
...
class CopyWindow : public Activity {
    ...
    void copyWindowContents() {
        ...
        sf::View newView = sf::View(sf::FloatRect((float)viewportIntRect.left, (float)viewportIntRect.top,
                                                  (float)viewportIntRect.width, (float)viewportIntRect.height));
...
template <int val = 0>
struct seconds {
    static sf::Time value() {
        return sf::seconds(val);
    }
};
